...
run_test<- function(data,numcoeffs,sigma_Y=1,shift=1.96,L=6.5,numgrid=3000,boots=150){
  ...
  for (b in 1:boots){
    ...
    n                      <- length(data$t)
    estar                  <- sqrt(n)*(coeffs_boot - orig_coeffs)
    #v_boot                 <- projpoint+estar
    resids_boot[b]         <- compute_residual_fast(estar,solver_tcone,alpha_start=0*projection$alpha_opt)$residual /sqrt(n)
    #use the tangent cone method
